#!/usr/bin/env bash
# push_aureum.sh -- Aureum private repo (aureum)
#
# Pushes ONLY the Aureum engineering content:
...
echo "==> Assembling Aureum export ..."

for DIR in AUREUM_REPO AUREUM_STAGE M_DRAFT M_FINAL; do
...
**Private** -- Aureum Engineering / BDP Phase Reversal / P vs NP Tower
...
Previously named: morningstar_spacecraft

## Contents

| Directory | Contents |
|-----------|----------|
| AUREUM_REPO/ | Full Aureum repo (BDP, MS tower, Lean stubs) |
| AUREUM_STAGE/ | Staging area for in-progress work |
...
The opera-numerorum repo (public) contains zero BDP/Aureum content.
...
git commit -q -m "Aureum -- ${COMMIT_DATE}

BDP Phase Reversal, P vs NP Tower, Aureum engineering.
