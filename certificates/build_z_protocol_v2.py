...
# ---- Load invariants.json at build time (no fabricated hashes) ----
INVARIANTS_PATH = "certificates/invariants.json"
with open(INVARIANTS_PATH) as _f:
    _INV = json.load(_f)

# Build flat lookup: top-level key -> entry, and "id" field -> entry
_LOOKUP = {}
for _k, _v in _INV.items():
    if isinstance(_v, dict):
        _LOOKUP[_k] = _v
        if "id" in _v:
            _LOOKUP[str(_v["id"])] = _v

def _get_sha(module_id):
    """Read stdout SHA from invariants.json. No fabricated values."""
    entry = _LOOKUP.get(module_id, {})
    for field in ["sha256_stdout", "stdout_sha256", "stdout_sha",
                  "sha256_pdf", "pdf_sha256", "pdf_sha"]:
        if field in entry:
            return entry[field]
    # Fallback: scan all entries for matching id
    for _v in _INV.values():
        if isinstance(_v, dict) and str(_v.get("id", "")) == module_id:
            for field in ["sha256_stdout", "stdout_sha256", "stdout_sha",
                          "sha256_pdf", "pdf_sha256", "pdf_sha"]:
                if field in _v:
                    return _v[field]
...
def _get_title(key, max_chars=70):
    """Read title or claim from invariants.json entry. Falls back to key name."""
...
# ---- 17 parent modules for Sectio XIV (causal order) ----
...
_SRC_SHA = {
    "Z2": _get_sha("module_6"),              # I. Galois -- M6 is genesis
    "Z3": _get_sha("module_m8c"),            # II. Modularis -- M8C
    "Z4": _get_sha("module_8"),              # III. Spectralis -- M8
    "Z5": _get_sha("module_m8j"),            # IV. Constructio -- M8J
    "Z6": _get_sha("module_23"),             # V. Aequidistributio -- M23
    "Z7": _get_sha("module_m8h"),            # VI. Formula Explicita -- M8H
}
...
if _MISSING or _MISSING_SRC:
    lines = ["BUILD ABORTED -- SHA not found in invariants.json:"]
    for label, key in _MISSING:
        lines.append("  Parent module {} (key: {})".format(label, key))
    for tbl, _ in _MISSING_SRC:
        lines.append("  Theorem source table {}".format(tbl))
    raise RuntimeError("\n".join(lines))
...
sigil_style = ParagraphStyle("sigil",
    fontName="Courier", fontSize=8, leading=10.5,
    alignment=TA_CENTER, textColor=SEPIA, spaceAfter=1)
...
def sig(text):
    return Paragraph(text, sigil_style)
...
# ============================================================
# ASCII MORNING STAR SIGIL -- 8-pointed star, pure ASCII
# Rendered in Courier, SEPIA ink.
# N/S/E/W + NE/NW/SE/SW = 8 rays. No torus. No toroid.
# ============================================================
...
def morning_star_sigil():
    """Return story elements for the ASCII Morning Star sigil."""
    items = []
    for line in MORNING_STAR_LINES:
        items.append(sig(line))
    return items
...
# --- Morning Star sigil on title page ---
story += morning_star_sigil()
story += [s(8)]
...
story += [HR_gold()]
story += [s(4)]
story += [Paragraph("THE Z PROTOCOL  --  v2", euler_sub)]
...
story += [Paragraph("v2: Morningstar sigil + theorem sources + Sectio XIV bibliography",
                    series_style)]
story += [PageBreak()]
...
story += [Paragraph(
    ...
    "All theorem source SHAs are read from certificates/invariants.json at build time.  "
...
story += [body(
    ...
    "each with SHA-traceable source citations; "
...
# (v2 addition: Source row cites M6 | SHA from invariants.json)
# ============================================================
...
z2_data = [
    ...
    ["Source",                     "module_6 | SHA: " + _z2_src[:16] + "... | Opera Numerorum",
                                                          "M6",     _z2_src[:8]],
]
z2 = Table(z2_data, colWidths=[2.0*inch, 2.2*inch, 0.8*inch, 0.75*inch])
z2_ts = tbl_style(NAVY)
z2_ts.add("BACKGROUND", (0, len(z2_data)-1), (-1, len(z2_data)-1),
          colors.HexColor("#e8f0e8"))
z2_ts.add("FONTNAME", (0, len(z2_data)-1), (0, len(z2_data)-1), "Courier-Bold")
z2.setStyle(z2_ts)
story += [z2]
story += [s(4)]
...
# (v2 addition: Source row cites M8C | SHA from invariants.json)
# ============================================================
...
z3_data = [
    ...
    ["Source",                   "module_m8c | SHA: " + _z3_src[:16] + "... | Opera Numerorum",
                                                         "M8C",    _z3_src[:8]],
]
z3 = Table(z3_data, colWidths=[1.9*inch, 2.1*inch, 0.95*inch, 0.75*inch])
z3_ts = tbl_style(SEPIA)
z3_ts.add("BACKGROUND", (0, len(z3_data)-1), (-1, len(z3_data)-1),
          colors.HexColor("#e8f0e8"))
z3_ts.add("FONTNAME", (0, len(z3_data)-1), (0, len(z3_data)-1), "Courier-Bold")
z3.setStyle(z3_ts)
story += [z3]
story += [s(4)]
...
# (v2 addition: Source row cites M8 | SHA from invariants.json)
# ============================================================
...
z4_data = [
    ...
    ["Source",                        "module_8 | SHA: " + _z4_src[:16] + "... | Opera Numerorum",
                                                                   "M8",     _z4_src[:8]],
]
z4 = Table(z4_data, colWidths=[1.9*inch, 2.1*inch, 0.9*inch, 0.75*inch])
z4_ts = tbl_style(NAVY)
z4_ts.add("BACKGROUND", (0, len(z4_data)-1), (-1, len(z4_data)-1),
          colors.HexColor("#e8f0e8"))
z4_ts.add("FONTNAME", (0, len(z4_data)-1), (0, len(z4_data)-1), "Courier-Bold")
z4.setStyle(z4_ts)
story += [z4]
story += [s(4)]
...
# (v2 addition: Source row cites M8J | SHA from invariants.json)
# ============================================================
...
z5_data = [
    ...
    ["Source",                  "module_m8j | SHA: " + _z5_src[:16] + "... | Opera Numerorum",
                                                           "M8J",       _z5_src[:8]],
]
...
z5_ts.add("BACKGROUND", (0, len(z5_data)-1), (-1, len(z5_data)-1),
          colors.HexColor("#e8f0e8"))
z5_ts.add("FONTNAME", (0, len(z5_data)-1), (0, len(z5_data)-1), "Courier-Bold")
z5.setStyle(z5_ts)
story += [z5]
story += [s(4)]
...
# (v2 addition: Source row cites M23 | SHA from invariants.json)
# ============================================================
...
z6_data = [
    ...
    ["Source",                     "M23 | SHA: " + _z6_src[:16] + "... | Opera Numerorum",
                                                                 "M23",    _z6_src[:8]],
]
...
z6_ts.add("BACKGROUND", (0, len(z6_data)-1), (-1, len(z6_data)-1),
          colors.HexColor("#e8f0e8"))
z6_ts.add("FONTNAME", (0, len(z6_data)-1), (0, len(z6_data)-1), "Courier-Bold")
z6.setStyle(z6_ts)
story += [z6]
story += [s(4)]
...
# (v2 addition: Source row cites M8H | SHA from invariants.json)
# ============================================================
...
z7_data = [
    ...
    ["Source",                     "module_m8h | SHA: " + _z7_src[:16] + "... | Opera Numerorum",
                                                              "M8H",     _z7_src[:8]],
]
...
z7_ts.add("BACKGROUND", (0, len(z7_data)-1), (-1, len(z7_data)-1),
          colors.HexColor("#e8f0e8"))
z7_ts.add("FONTNAME", (0, len(z7_data)-1), (0, len(z7_data)-1), "Courier-Bold")
z7.setStyle(z7_ts)
story += [z7]
story += [s(4)]
...
story += [sec("XIV.  SECTIO XIV  --  BIBLIOGRAPHIA  (Table Z14)")]
story += [HR()]
...
story += [body(
    ...
    "No hash is fabricated.  No module is omitted from this chain."
)]
story += [s(6)]
...
for label, inv_key, _fallback_desc in PARENT_MODULES_17:
    ...
    if sha == "NOT_FOUND_IN_INVARIANTS":
        ...
        sha_display = sha[:32]
        st = "CERTIFIED"
    desc = _get_title(inv_key)
    z14_data.append([label, desc, sha_display, st])
...
for i, (label, _, _) in enumerate(PARENT_MODULES_17, start=1):
    if label in ("M1", "M6", "M8", "M23", "M9"):
        z14_ts.add("BACKGROUND", (0, i), (-1, i), colors.HexColor("#f0e8d0"))
z14.setStyle(z14_ts)
story += [z14]
story += [s(4)]
_z14_sha = table_sha(z14_data)
story += [frozen("TABLE Z14  --  BIBLIOGRAPHIA  --  2026-06-04  --  SHA: " + _z14_sha[:8] + "  --  SORRY: 0")]
story += [s(6)]
...
# ============================================================
# BACK PAGE: FINAL CERTIFICATION + COLOPHON SIGIL
# ============================================================
...
# --- Colophon: Morning Star sigil on final page ---
story += [HR_sepia()]
story += [s(4)]
story += morning_star_sigil()
story += [s(4)]
